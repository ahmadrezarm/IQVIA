...
for folder in folders:
    ...
    for item in os.listdir(dir_name):  # loop through items in dir
        if item.endswith(extension):  # check for ".csv" extension
            ...
            #print(df[["9"]].dtypes)
            #print(df.columns)
            len_totoal = len_totoal + len(df.loc[df['3'] == 'P'])
            # now we have the csv file. Lets filter things out:
            no_anti_inflammatry_df = pd.concat([no_anti_inflammatry_df, df.loc[~df['9'].isin(code_list), :]],ignore_index=True)
            print(len(no_anti_inflammatry_df))
