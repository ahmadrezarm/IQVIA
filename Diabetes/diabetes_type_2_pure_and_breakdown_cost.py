...
#ComCliamEnrollDF[type_1_list] = 0
ComCliamEnrollDF[type_1_list] = ComCliamEnrollDF.iloc[:, 22:34].apply(lambda x: pd.Series(np.isin(type_1_list, x).astype(int)), axis=1)
'''
...
ComCliamEnrollDF['type_1'] = (ComCliamEnrollDF[type_1_list].sum(axis=1) > 0).astype(int)
print(ComCliamEnrollDF['type_1'].value_counts())

# the target:
ComCliamEnrollDF['type_2'] = 0
ComCliamEnrollDF['type_2'] = ((ComCliamEnrollDF[type_2_list].sum(axis=1) > 0) & (ComCliamEnrollDF['type_1'] ==0)).astype(int)
print(ComCliamEnrollDF['type_2'].value_counts())
print('Here here')

(ComCliamEnrollDF.groupby(['type_2', 'year'])['allowed'].sum() / ComCliamEnrollDF.groupby(['type_2', 'year'])['pat_id'].nunique()).to_csv('E:/Diabetes Project/Analysis results/type_2_pure_total_cost.csv')
...
diabetes_type_2_DF = ComCliamEnrollDF.loc[(ComCliamEnrollDF['type_2'] == 1), :]

DirectComCliamEnrollDF = diabetes_type_2_DF
...
#############################################################
        ...
        print(" I looped once!")
TotalInfoDF.to_csv(f"E:/Diabetes Project/Analysis results/type_2_cost_breakdown.csv",index=None)
